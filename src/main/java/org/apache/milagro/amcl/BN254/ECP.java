...
//		if (INF) return true;                            // Edits made
	...
	public void toBytes(byte[] b, boolean compress) {
		...
		W.x.redc().toBytes(t);
		System.arraycopy(t, 0, b, 1, BIG.MODBYTES);
	...
	public static ECP fromBytes(byte[] b) {
		...
		System.arraycopy(b, 1, t, 0, BIG.MODBYTES);
	...
	public static ECP fromHex(String hex) {
		String[] values = hex.split(" ");
		FP[] fps = new FP[values.length / 2];
		for (int i = 0; i < fps.length; i++) {
			fps[i] = FP.fromHex(String.format("%s %s", values[2 * i], values[2 * i + 1]));
		}
		return new ECP(fps[0], fps[1], fps[2]);
	}

	public String toHex() {
		return String.format("%s %s %s", x.toHex(), y.toHex(), z.toHex());
	}
