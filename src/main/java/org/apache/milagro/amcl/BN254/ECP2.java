...
//		if (is_infinity()) return;
	...
	public static ECP2 fromHex(String hex){
		String[] values = hex.split(" ");
		FP2[] fps = new FP2[values.length / 4];
		for(int i = 0; i < fps.length; i ++){
			fps[i] = FP2.fromHex(String.format("%s %s %s %s",
					values[4 * i], values[4 * i + 1],
					values[4 * i + 2], values[4 * i + 3]));
		}
		return new ECP2(fps[0], fps[1], fps[2]);
	}
