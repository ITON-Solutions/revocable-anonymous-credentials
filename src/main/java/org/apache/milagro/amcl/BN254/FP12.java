...
public final class FP12 {
	...
	public static FP12 fromHex(String hex){
		String[] values = hex.split(" ");
		FP[] fps = new FP[values.length / 2];
		for(int i = 0; i < fps.length; i ++){
			fps[i] = FP.fromHex(String.format("%s %s", values[2 * i], values[2 * i + 1]));
		}
		return new FP12(new FP4(new FP2(fps[0], fps[1]), new FP2(fps[2], fps[3])),
				new FP4(new FP2(fps[4], fps[5]), new FP2(fps[6], fps[7])),
				new FP4(new FP2(fps[8], fps[9]), new FP2(fps[10], fps[11])));
	}

	public String toHex() {
		return String.format("%s %s %s", a.toHex(), b.toHex(), c.toHex());
	}
