...
public class BIG {
	...
	public static final int NLEN = (1 + ((8 * MODBYTES - 1) / BASEBITS)); // 5
	...
	public static final int NEXCESS = (1 << (CHUNK - BASEBITS - 1));
	...
	public BIG(BIG x) {
		System.arraycopy(x.w, 0, w, 0, NLEN);
	}
	...
	public String toRawString() {
		...
		for (int i = 0; i < NLEN - 1; i++) {
			s.append(Long.toHexString(b.w[i]));
			s.append(",");
		}
		s.append(Long.toHexString(b.w[NLEN - 1]));
		s.append(")");
		return s.toString();
	}
	...
	public static BIG fromHex(String[] data) {
		long[] longs = new long[NLEN];
		for(int i = 0; i < longs.length; i++){
			longs[i] = Long.parseLong(data[i], 16);
		}
		return new BIG(longs);
	}
	...
	public String toString() {
		...
		for (int i = len - 1; i >= 0; i--) {
			b = new BIG(this);
			b.shr(i * 4);
			s.append(Long.toHexString(b.w[0] & 15));
		}
		return s.toString();
	}
	...
	static BIG monty(BIG md, long MC, DBIG d) {
		...
		b = new BIG(0);
		System.arraycopy(d.w, 5, b.w, 0, NLEN);
		b.norm();
		return b;
	}
