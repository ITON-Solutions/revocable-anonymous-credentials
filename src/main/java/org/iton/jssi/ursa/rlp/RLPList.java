...
public final class RLPList extends RLPItem {
    ...
    RLPList(byte lead, RLPDataType type, byte[] buffer, int index, int containerEnd, boolean lenient) throws DecodeException {
    ...
    static RLPList withElements(Iterable<RLPItem> srcElements) {
        ...
        if (dataLen < RLPDataType.MIN_LONG_DATA_LEN) {
            ...
            dest[0] = (byte) (RLPDataType.LIST_SHORT_OFFSET + dataLen);
        ...
        try {
            byte lead = dest[0];
            return new RLPList(lead, RLPDataType.type(lead), dest, 0, dest.length, false);
    ...
    private static byte[] encodeListLong(final int srcDataLen, final Iterable<RLPItem> srcElements) {
        ...
        dest[0] = (byte) (RLPDataType.LIST_LONG_OFFSET + n);
